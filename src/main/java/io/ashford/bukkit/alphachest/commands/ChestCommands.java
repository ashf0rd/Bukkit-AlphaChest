package io.ashford.bukkit.alphachest.commands;

import io.ashford.bukkit.alphachest.Teller;
import io.ashford.bukkit.alphachest.VirtualChestManager;
import io.ashford.bukkit.alphachest.Teller.Type;
...
public class ChestCommands implements CommandExecutor {
	...
	private boolean performChestCommand(CommandSender sender, String[] args) {
		if (sender instanceof Player) {
			...
			if (player.getGameMode().equals(GameMode.CREATIVE)
					&& !player.hasPermission("alphachest.chest.creativeMode")) {
				Teller.tell(sender, Type.Error,
						"You are not allowed to open your chest in Creative Mode!");
				return true;
			}
			...
			if (args.length == 0) {
				...
				if (sender.hasPermission("alphachest.chest")) {
					Inventory chest = chestManager.getChest(((Player) sender).getName());
				...
				if (sender.hasPermission("alphachest.admin")) {
					...
					Teller.tell(sender, Type.Error,
							"You are not allowed to open other user's chests.");
				}
				return true;
			}
			...
			Teller.tell(sender, Type.Error,
					"Only players are able to open chests.");
			return true;
		}
	}
	...
	private boolean performClearChestCommand(CommandSender sender, String[] args) {
		if (args.length == 0 && sender instanceof Player) {
			if (sender.hasPermission("alphachest.chest")) {
				chestManager.removeChest(((Player) sender).getUniqueId().toString());
				Teller.tell(sender, Type.Success,
						"Successfully cleared your chest.");
			} else {
				Teller.tell(sender, Type.Error,
						"You are not allowed to use this command.");
			}
			...
			if (sender.hasPermission("alphachest.admin")) {
				...
				for (OfflinePlayer target : Bukkit.getOfflinePlayers()) {
					// Search all players known to the server for the name
					// specified
					if (target.getName().equalsIgnoreCase(args[0])) {
						flagNotFound = false;
						chestManager.removeChest(target.getUniqueId().toString());
						Teller.tell(sender, Type.Success,
								"Successfully cleared " + args[0]
										+ "\'s chest.");
					}
				}
				if (flagNotFound) {
					Teller.tell(sender, Type.Error,
							String.format("Chest for %s not found", args[0]));
				}
			} else {
				Teller.tell(sender, Type.Error,
						"You are not allowed to clear other user's chests.");
			}
			return true;
		} else {
			return false;
	...
	private boolean performSaveChestsCommand(CommandSender sender, String[] args) {
		if (sender.hasPermission("alphachest.save")) {
			...
			Teller.tell(sender, Type.Success, "Saved " + savedChests
					+ " chests.");
		} else {
			Teller.tell(sender, Type.Error,
					"You are not allowed to use this command.");
